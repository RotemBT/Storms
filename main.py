...
def crawling_table():
    ...
    for url in all_url:
        ...
        for td in tdOfYear:
            ...
            tdOfDates = bs.find_all('td', {
                'class': 'mat-cell cdk-cell cdk-column-designatedTrack mat-column-designatedTrack ng-star-inserted'})
            for i in tdStormsNames:
                name = i.find('a').text
                storms_names.append(i.find('a').text)

                tdLat= bs.find_all('td',{'class':'mat-cell cdk-cell cdk-column-lat mat-column-lat ng-star-inserted'})

                # problem to move to correct link, there is no pattern in the suffix
                
                next_page = url + "/" + td.find('a').text + "/Hurricane-"+ i.find('a').text
                # next_page = url + "/" + td.find('a').text + "/Hurricane-" + "{fname}".format(fname=name)
                # https://www.wunderground.com/hurricane/archive/AL/2018/Hurricane-Kirk/2018
                print(next_page)
                ...
                bs = BeautifulSoup(c, "html.parser")
                for i in tdLat:
                    lat_sum.append(i.find('span').text)
                lat_avg= sum(lat_sum)/ len(lat_sum)
                print(lat_avg)
    ...
    d2 = {'storms_names': storms_names, 'month':month, 'max_winds':maxWinds, 'max_strength':storm_type}
